...
export function generateCertificate(data: CertificateData) {
  ...
  // ── Decorative border ──
  doc.setDrawColor(16, 102, 79); // primary-600
  doc.setLineWidth(2);
  ...
  doc.text("VA", w / 2, 31, { align: "center" });

  // ── Title ──
  doc.setTextColor(16, 102, 79);
  doc.setFontSize(14);
  doc.setFont("helvetica", "normal");
  doc.text("V-ALIGN LMS", w / 2, 44, { align: "center" });
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "normal");
  doc.text("has successfully completed the course", w / 2, 96, {
    align: "center",
  });
  ...
  doc.setFontSize(8);
  doc.text(`Certificate ID: ${data.certificateId}`, w / 2, h - 20, {
    align: "center",
  });
  ...
  // ── Download ──
  const filename = `VAlign-Certificate-${data.courseName.replace(/\s+/g, "-")}.pdf`;
  doc.save(filename);
}
